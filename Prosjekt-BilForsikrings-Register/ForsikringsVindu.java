/* Konstruerer GUI og sأ¸rger for all knappefunksjon.
   Koden er redusert til أ¥ kun omhhandle bilforsikring slik som vi ble bedt om,
...
public class ForsikringsVindu extends JFrame
{
  ...
  //	                   verdifelt, sumfelt;
  private JTextField biltypefelt,regأ¥rfelt, regnrfelt,
  ...
  public ForsikringsVindu()
  {
    ...
    regأ¥rfelt = new JTextField(20);
	regأ¥rfelt.addActionListener(lytter);
	p9.add(new JLabel("Registreringsأ¥r "));
	p9.add(regأ¥rfelt);
    vestpanel.add(p9);
	...
	p11.add(new JLabel("أ…rlig kjأ¸relengde "));
	p11.add(lengdefelt);
	vestpanel.add(p11);
...
*/
    ...
    //Metode for أ¥ registrere en ny kunde
    ...
    //Metode for أ¥ finne en gitt kunde
    ...
    //Metode for أ¥ slette en gitt kunde
    ...
    //Gأ…R ANN أ… REGISTRERE EN FORSIKRING UTEN REGNR, MEN IKKE UTEN NOE ANNET?!??! FIKS
    //Registrerer en ny BilForikring pأ¥ gitt bruker
    public void tegnBilForsikring(ForsikringsKunde k) {
      try {
       
        String type = biltypefelt.getText(); 
        int regأ¥r = Integer.parseInt(regأ¥rfelt.getText());
        ...
        BilForsikring bilForsikring = new BilForsikring(type, regأ¥r, regnr, lengde, bonus);
    ...
    //Lager en ny forsikring utifra hvilken knapp som ble trykket pأ¥
